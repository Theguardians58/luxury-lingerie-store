...
export default async function HomePage() {
  ...
  // Fetch the first 4 products to feature on the homepage
  const { data: featuredProducts, error } = await supabase
    .from('products')
    .select(`*, categories (name), product_images (image_url, alt_text)`)
    .limit(4);
  ...
  return (
    <div>
      {/* Hero Section */}
      <section className="relative h-[60vh] md:h-[80vh] bg-cover bg-center text-white flex items-center justify-center" style={{ backgroundImage: "url('https://i.imgur.com/gBw9Y8E.jpeg')" }}>
          ...
          {(featuredProducts && featuredProducts.length > 0) ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {featuredProducts.map(product => (
                // We cast the product here to ensure it matches the expected type
                <ProductCard key={product.id} product={product as ProductWithDetails} />
              ))}
            </div>
          ) : (
