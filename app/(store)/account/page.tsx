...
export default async function AccountPage({ searchParams }: { searchParams: { message?: string }}) {
  ...
  // THE FINAL FIX (Part 1): We cast the profile data to 'any' immediately.
  ...
  async function updateUserProfile(formData: FormData) {
    ...
    // THE FINAL FIX (Part 2): We cast the update object to 'any'.
    // This overrides the stubborn 'never' error for the update function.
    const { error } = await supabase
      ...
      } as any) // This is the definitive fix for the update error.
      .eq('id', user.id);
